...
('ERP-DEMO-2026-001','华南智能制造 ERP 上线项目（模拟）',1,100000.00,'implementation',CURDATE() - INTERVAL 20 DAY,CURDATE() + INTERVAL 15 DAY);

INSERT INTO payment_milestones(project_id, milestone_name, percentage, planned_amount, status, due_date, paid_date) VALUES
(1,'签约款',30,30000.00,'paid',CURDATE() - INTERVAL 15 DAY,CURDATE() - INTERVAL 14 DAY),
(1,'上线款',40,40000.00,'invoiced',CURDATE() + INTERVAL 5 DAY,NULL),
(1,'验收款',30,30000.00,'pending',CURDATE() + INTERVAL 35 DAY,NULL);
